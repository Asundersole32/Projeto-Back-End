...
class ListMachineGameObjectView(generics.ListAPIView):
    permission_classes = [IsAuthenticated]
    serializer_class = MachineGameObjectSerializer

    def get(self, request):
        try:
            machine_game_object = MachineGameObject.objects.all()
            serializer = MachineGameObjectSerializer(machine_game_object, many=True)
