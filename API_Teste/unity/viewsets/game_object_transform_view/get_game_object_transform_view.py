...
class GetGameObjectTransformView(generics.RetrieveAPIView):
    permission_classes = [IsAuthenticated]
    serializer_class = GameObjectTransformSerializer

    def get(self, request, game_object_transform_id=None):
        try:
            game_object_transform = GameObjectTransform.objects.get(pk=game_object_transform_id)
            serializer = GameObjectTransformSerializer(game_object_transform)
